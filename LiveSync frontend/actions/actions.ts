...
export async function createNewDocument (){
    ...
    .collection('rooms').doc(docRef.id).set({
         userId : sessionClaims?.email!, //use this as unique identifier
         role:"owner",
         createdAt : new Date(),
         roomId: docRef.id
    });
    ...
    export async function deleteDocument(roomId :string) {  
       ...
       // console.log("deleteDocument",roomId);
        ...
        try{
            ...
            //finding all of the places where the room "abc " matches
            const query = await adminDb.collectionGroup("rooms").where("roomId","==",roomId).get();
            
            //make batch 
            const batch = adminDb.batch();
             
            //delete the room reference in the user's collection for every user who is in the room
